...
from .backbones.vim_official import vim_small_patch16_224_FSRA
...
class build_transformer(nn.Module):
    def __init__(self, opt,num_classes,block = 4 ,return_f=False):
        ...
        elif opt.backbone=="vim_small":
            # VIM (Vision Mamba) backbone
            transformer_name = "vim_small_patch16_224_FSRA"
            self.in_planes = 384  # VIM small has 384 embed_dim
            print('using Transformer_type: {} as a backbone'.format(transformer_name))
            
            self.transformer = vim_small_patch16_224_FSRA(img_size=(256,256), stride_size=[16, 16], drop_path_rate=0.1,
                                                            drop_rate= 0.0, attn_drop_rate=0.0)
            self.transformer.load_param(model_path)
